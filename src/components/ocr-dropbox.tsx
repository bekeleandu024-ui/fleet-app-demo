...
async function loadTesseract(): Promise<TesseractLike> {
  if (!tesseractPromise) {
    tesseractPromise = import("tesseract.js").then(m =>
      (m as any).default ?? (m as any as TesseractLike)
    );
  }
...
export default function OcrDropBox({
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      className="border-2 border-dashed rounded p-4 text-sm text-gray-600"
      aria-busy={busy}
      role="region"
    >
